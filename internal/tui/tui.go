...
func updateReport(m model, msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "p":
			...
			hasDupesToPurge := (m.finalReport != nil && !m.finalReport.Summary.IsValidationReport) && ((m.purgeIds && len(m.finalReport.DuplicateIDs) > 0) || (m.purgeRows && len(m.finalReport.DuplicateRows) > 0))
			if !isGCS && hasDupesToPurge && m.purgeStats.filesModified == 0 {
				if m.purgeIds {
...
func renderInputPath(m *model) string {
	...
	help := helpStyle.Render("Press Enter to submit, 'q' or 'ctrl+c' to quit.")
...
func renderReport(m *model) string {
	...
	isGCS := strings.Contains(m.path, "gs://")
	hasDupes := (m.finalReport != nil && !m.finalReport.Summary.IsValidationReport && len(m.finalReport.DuplicateIDs) > 0 && m.purgeIds) || (m.finalReport != nil && !m.finalReport.Summary.IsValidationReport && len(m.finalReport.DuplicateRows) > 0 && m.purgeRows)

	if !isGCS && hasDupes && m.purgeStats.filesModified == 0 {
